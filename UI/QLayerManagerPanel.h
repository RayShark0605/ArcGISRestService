...
struct LayerManagerChangeInfo
{
	...
	std::vector<LayerManagerLayerInfo> allLayers;      // 变化后的全部图层状态。
	std::vector<LayerManagerLayerInfo> affectedLayers; // 本次动作直接影响的图层。
	std::vector<int> affectedRows;                     // 本次动作直接影响的行号；移除动作中为移除前行号。
 ...
 * 面板本身不直接操作主画布；当用户改变显示状态、请求缩放或请求移除图层时，
...
private:
	...
	void RemoveRows(const std::vector<int>& rowsToRemove, LayerManagerActionType actionType);
