#ifndef GEOBASE_READ_WRITE_LOCK_H_H
#define GEOBASE_READ_WRITE_LOCK_H_H
...
*/
class ARCGIS_RESTSERVICE_PORT GB_ReadWriteLock
{
public:
...
/*
	...
	- 通过不同的 tag 类型参与重载，调用者可以在构造时明确选择策略：
		...
		3) 尝试加锁不阻塞（TryToLock）
*/
struct ARCGIS_RESTSERVICE_PORT GB_DeferLockTag {};
struct ARCGIS_RESTSERVICE_PORT GB_TryToLockTag {};
...
*/
class ARCGIS_RESTSERVICE_PORT GB_ReadLockGuard
{
public:
	// 立即加锁（阻塞）
...
/*
	...
	- 持有期间保持“写锁”，析构时自动释放。
*/
class ARCGIS_RESTSERVICE_PORT GB_WriteLockGuard
{
public:
	// 立即加锁（阻塞）
